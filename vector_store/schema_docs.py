...
SCHEMA_DOCS: list[dict] = [
    ...
    },
    {
        ...
        "content": (
            ...
            "Q: 'Procurement-to-payment trail for supplier Deloitte — POs, receipts, invoices, and payments'\n"
            "A: SELECT s.supplier_name, po.po_number, po.total_amount as po_amount,\n"
            "     rh.receipt_number, rh.receipt_date,\n"
            "     i.invoice_number, i.invoice_amount, i.status as invoice_status,\n"
            "     p.payment_number, p.payment_amount, p.payment_date, p.status as payment_status\n"
            "   FROM sup_suppliers s\n"
            "   LEFT JOIN proc_po_headers po ON po.supplier_id = s.supplier_id\n"
            "   LEFT JOIN proc_receipt_headers rh ON rh.po_header_id = po.po_header_id\n"
            "   LEFT JOIN fin_ap_invoices i ON i.supplier_id = s.supplier_id\n"
            "   LEFT JOIN fin_ap_payments p ON p.supplier_id = s.supplier_id\n"
            "   WHERE s.supplier_name ILIKE '%Deloitte%'\n"
            "   ORDER BY po.po_number, i.invoice_date LIMIT 100\n\n"
    ...
    # CROSS-DOMAIN DOCUMENTATION
    {
        ...
        "content": (
            ...
            "        PO allocation to cost centres lives in proc_po_distributions.\n\n"
            "=== VERIFIED JOIN PATHS ===\n\n"
            "1. Headcount by Department (CORRECT path):\n"
            "   fin_cost_centres cc → hcm_departments d (via cc.dept_id = d.dept_id)\n"
            ...
            "3. PO Spend by Department (CORRECT path — NOT via requisition_id):\n"
            ...
            "   pod → proc_po_headers po (via po.po_header_id = pol.po_header_id)\n"
            ...
            "     SUM(gb.actual_amount - gb.budget_amount) AS budget_variance,\n"
            ...
            "   GROUP BY d.dept_name, cc.cost_centre_code ORDER BY d.dept_name\n\n"
            "Q: 'Show headcount per cost centre with budget utilization'\n"
            "A: SELECT cc.cost_centre_name,\n"
            "     COUNT(DISTINCT a.person_id) AS headcount,\n"
            "     SUM(gb.actual_amount) AS actual, SUM(gb.budget_amount) AS budget\n"
            "   FROM fin_cost_centres cc\n"
            "   JOIN hcm_departments d ON d.dept_id = cc.dept_id\n"
            "   LEFT JOIN hcm_assignments a ON a.dept_id = d.dept_id AND a.assignment_status = 'ACTIVE'\n"
            "   LEFT JOIN fin_gl_balances gb ON gb.cost_centre_id = cc.cost_centre_id\n"
